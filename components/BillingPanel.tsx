...
export default function BillingPanel() {
  ...
  const rows = [
    [t('长期问题', 'Long-lived questions'), t('3 个 · 待启用套餐限制', '3 · plan limit pending'), t('10 个 · 待启用套餐限制', '10 · plan limit pending')],
    ...
    [t('月度研究额度（待启用）', 'Monthly research runs (planned)'), `${plans.free.researchRuns}${t(' 次', ' runs')}`, `${plans.pro.researchRuns}${t(' 次', ' runs')}`],
    [t('周期观察（套餐频率待启用）', 'Scheduled observation (plan schedules pending)'), t('每周', 'Weekly'), t('每天，可调低', 'Daily, adjustable')],
  ];
  return <section className="text-slate-200 space-y-5" aria-label={t('收费与套餐', 'Plans & billing')}>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div className="p-5 rounded-2xl bg-blue-600/10 border border-blue-500/50"><h3 className="font-bold text-xl text-blue-300">Pro</h3><p className="text-3xl font-bold mt-3">¥{plans.pro.amount / 100}<span className="text-sm text-slate-400 font-normal">{t('／月', '/month')}</span></p><p className="text-sm text-slate-400 mt-2">{t('更多研究空间，成果随时带走', 'More room for research, with exports included')}</p><button disabled={!canPay || busy || !!order} onClick={() => void start()} className="w-full mt-4 px-4 py-2 rounded-lg bg-blue-600 disabled:bg-slate-700 disabled:text-slate-400 text-sm font-bold">{busy ? t('正在创建订单…', 'Creating order…') : !canPay ? t('Pro 暂未开放购买', 'Pro purchases are not open yet') : account?.plan === 'pro' ? t('续费一个月', 'Add one month') : t('购买 Pro · ¥69', 'Buy Pro · ¥69')}</button></div>
    </div>
    ...
    <p className="text-xs text-slate-400 leading-relaxed">{t('套餐研究次数与观察频率仍在接入，当前以模型实际额度为准。长期保留不等于无限计算；没有新依据时安静等待。商户收款和权益交付验收完成前，不开放 Pro 购买。', 'Research allowances and schedules are being integrated; current model balances still apply. Long-lived questions do not mean unlimited computation. Pro sales stay closed until payments and benefits are ready.')}</p>
