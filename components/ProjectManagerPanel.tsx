...
export default function ProjectManagerPanel({ project, teams, onTeam }: {
  ...
  return <section aria-label="AI 项目经理" className="rounded-xl border border-blue-500/30 bg-slate-900 p-4 space-y-3">
    <div className="flex justify-between gap-3"><div><h3 className="font-semibold text-sm">🧭 {profile.name}</h3><p className="text-xs text-blue-300 mt-1">{profile.model.model || '跟随全局模型'} · 项目内沟通与协调</p></div>{onTeam && <button className={button} onClick={onTeam}>配置经理和团队</button>}</div>
    <p className="text-xs text-slate-400">汇总各想法的进展、事实与审计意见。协调建议可直接启动或暂停对应团队；对话保存在项目探索阶段中。</p>
    <div ref={historyRef} className="max-h-96 overflow-y-auto space-y-3" aria-live="polite">
      {!messages.length && <p className="text-xs text-slate-500">可以告诉我你的目标、限制和新发现，或让我整理当前最值得验证的问题。</p>}
      {messages.map(message => <article key={message.id} className={`rounded-lg p-3 text-xs ${message.role === 'user' ? 'bg-blue-950/50' : 'bg-slate-800/70'}`}>
        <p className="text-[10px] text-slate-500 mb-2">{message.role === 'user' ? '你' : message.agentSnapshot?.name || profile.name} · {new Date(message.createdAt).toLocaleString('zh-CN')}</p>
        <p className="whitespace-pre-wrap leading-relaxed break-words">{message.content}</p>
        {message.actions?.map(action => <div key={action.id} className="mt-3 border-t border-slate-700 pt-2 space-y-2">
          <p className="text-blue-300">{action.type === 'start' ? '开始 / 继续团队' : '当前步骤后暂停'} · {action.questionId === 'root' ? project.metaProblem : project.nodes.find(n => n.id === action.questionId)?.title || '已删除想法'}</p>
    ...
    <form className="space-y-2" onSubmit={async e => { e.preventDefault(); const value = text; try { setError(''); await teams.sendManager(project.id, value); setText(''); } catch (err) { setError(err instanceof Error ? err.message : '沟通失败'); } }}>
      <textarea aria-label="给项目经理的消息" className="w-full rounded-lg border border-slate-700 bg-slate-950 p-3 text-sm focus:border-blue-400 focus:outline-none" rows={3} maxLength={6000} value={text} disabled={busy} onChange={e => setText(e.target.value)} placeholder="告诉项目经理你的目标、新证据，或需要协调的事情…" />
      <button className={`${button} bg-blue-600`} disabled={busy || !text.trim()} type="submit">{busy ? '项目经理正在梳理…' : '发送给项目经理'}</button>
    </form>
