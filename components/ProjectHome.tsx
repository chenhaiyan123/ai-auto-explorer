...
export default function ProjectHome({ project, scopeId, onPage, onNote, onGenerateReport, generating, onSaveBrief, onSaveSummary, manager, routePanel }: {
  ...
  return <div className="p-4 md:p-6 space-y-5 max-w-6xl mx-auto">
    <div className="rounded-2xl border border-blue-500/25 bg-gradient-to-br from-blue-950/60 to-slate-900 p-5">
      <div className="flex justify-between items-center gap-3 mb-3"><p className="text-xs text-blue-300">{scopeId === 'root' ? ui("项目总览") : ui("想法总览")}</p>{onGenerateReport && scopeId === 'root' && <button className="text-xs text-slate-300 border border-slate-600 rounded-lg px-3 py-2 disabled:opacity-40" disabled={generating} onClick={onGenerateReport}>{generating ? '正在生成…' : '生成研究报告'}</button>}</div>
    ...
    <div className="grid lg:grid-cols-2 gap-4">
      ...
      <section className="rounded-xl border border-slate-700 bg-slate-900/70 p-4 space-y-3"><div className="flex justify-between"><h3 className="text-sm font-semibold">已确认的事实</h3><button className="text-xs text-emerald-300" onClick={() => onPage('facts')}>事实看板 →</button></div>{confirmed.length === 0 && <p className="text-xs text-slate-500">目前还没有经核验的事实，AI 观点不会被当作事实展示。</p>}{confirmed.slice().sort((a, b) => b.updatedAt - a.updatedAt).slice(0, 5).map(f => <button key={f.id} className="block text-left w-full border-l-2 border-emerald-600 pl-3" onClick={() => onPage('facts', f.scopeId)}><p className="text-xs text-slate-200">{f.claim}</p><p className="text-[10px] text-slate-500 mt-1">{ui("适用范围：")}{f.scope}</p></button>)}</section>
    </div>
    ...
    {hypotheses.length > 0 && <section className="rounded-xl border border-violet-500/30 p-4 space-y-3"><h3 className="text-sm font-semibold">正在检验的假设</h3>{hypotheses.slice(0, 6).map(h => <button key={h.id} className="block w-full text-left text-xs text-slate-300" onClick={() => onPage('team', h.scopeId)}><p>{h.statement}</p><p className="text-slate-500 mt-1">{ui("证伪条件：")}{h.falsification}</p></button>)}</section>}
    <section className="rounded-xl border border-slate-700 p-4 space-y-3"><h3 className="text-sm font-semibold">笔记探索进度</h3><div className="flex flex-wrap gap-4 text-xs"><span className="text-emerald-300">{ui("已完成")}{dashboard.solved}</span><span className="text-blue-300">{ui("进行中")}{dashboard.exploring}</span><span className="text-slate-400">{ui("待探索")}{dashboard.unexplored}</span><span className="text-amber-300">等待现实验证 {dashboard.awaitingReality}</span><span className="text-rose-300">被推翻 / 待复核 {dashboard.contradicted + dashboard.review}</span></div><p className="text-[11px] text-slate-500">笔记完成状态与事实核验分别统计，写完一篇笔记不代表结论已被证实。</p>
